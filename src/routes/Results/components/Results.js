...
export class Results extends React.Component {
    ...
    return [
      {
        ...
        resultBlob: {
          log: [
            [1511874228304, "start"],
            [1511874238339, "reduce"],
            [1511874249860, "reduceEnd"],
            [1511874251872, "add"],
            [1511874263370, "addEnd"],
            [1511874266509, "submitBasketPreference"],
            [1511874284554, "finalSubmit"],
            [1511874299707, "finalSubmit"]
          ],
          survey: {
            age: "11",
            sex: "male",
            ...
            basketPreference: "a",
            adding_strategies: "1",
            removing_strategies: "1"
          },
          groupcode: "asegdsafgdsg",
          addedValues: [1, 6, 7, 9, 11, 12, 14, 15],
          reducedValues: [2, 6, 7, 8, 10, 11, 12, 15],
          addedBasketLog: [
            [1511874253515, "add", 6],
            [1511874254732, "add", 11],
            [1511874255821, "add", 9],
            [1511874257250, "add", 14],
            [1511874258623, "add", 7],
            [1511874259579, "add", 15],
            [1511874260760, "add", 12],
            [1511874262358, "add", 1]
          ],
          evaluationOrder: "reduce",
          initialAddOrder: [6, 4, 11, 9, 7, 14, 15, 1, 10, 12, 2, 5, 13, 8, 3],
          reducedBasketLog: [
            [1511874240354, "reduce", 9],
            [1511874241455, "reduce", 3],
            [1511874242670, "reduce", 1],
            [1511874244886, "reduce", 5],
            [1511874246337, "reduce", 4],
            [1511874247699, "reduce", 13],
            [1511874248869, "reduce", 14]
          ],
          orderOfProcedures: "reduce",
          initialReduceOrder: [
            9,
            3,
            1,
        ...
        createdAt: "2017-11-28T13:06:22.941Z",
        updatedAt: "2017-11-28T13:06:22.941Z"
      }
    ];
  };
  ...
  populateResults = data => {
    ...
    _.forEach(data, item => {
      ...
      toTempRow(
        _.intersection(
          item.resultBlob.addedValues,
          item.resultBlob.reducedValues
        ).length
      );
      // which came first
      ...
      if (item.resultBlob.initialAddOrder) {
        for (var k = 0; k < 15; k++) {
      ...
      if (item.resultBlob.initialReduceOrder) {
        for (var k = 0; k < 15; k++) {
      ...
      toTempRow(
        (findTime(logB, "reduceEnd") - findTime(logB, "reduce")) / 1000
      ); // reduce
      toTempRow(
        (findTime(logB, "finalSubmit") - beforeSubmitBasketPreference(logB)) /
          1000
      ); //survey
      toTempRow(
        (findTime(logB, "finalSubmit") - findTime(logB, "start")) / 1000
      ); // total
      ...
      toTempRow(
        _.groupBy(_.map(addedBasketLogObj, ret2ndValOfArr)).add
          ? _.groupBy(_.map(addedBasketLogObj, ret2ndValOfArr)).add.length
          : null
      );

      toTempRow(
        _.groupBy(_.map(reducedBasketLogObj, ret2ndValOfArr)).reduce
          ? _.groupBy(_.map(reducedBasketLogObj, ret2ndValOfArr)).reduce.length
          : null
      );
        ...
        <Table className="table">
          <thead>
            <tr>
              ...
              <th colSpan="10">
                {" "}
                Evaluation of procedures - Scale: 1, very easy/not at all - 5,
                very difficult/very much{" "}
              </th>
